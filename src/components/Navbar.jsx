...
const Navbar = () => {
  ...
  return (
    <>
      ...
      <nav className={`w-full bg-white/95 backdrop-blur-md border-b border-yellow-200/50 sticky top-0 z-50 transition-all duration-300 ${isScrolled ? 'shadow-lg shadow-red-100/50' : ''}`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex items-center justify-between">
          
          {/* Logo & Brand - Always Visible */}
          <div className="flex items-center gap-3 min-w-0 flex-shrink-0">
            <div className="h-12 w-12 rounded-full bg-gradient-to-br from-red-500 to-red-600 flex items-center justify-center text-white font-bold text-lg shadow-lg flex-shrink-0">
              TVK
            </div>
          ...
          >
            <svg className={`w-6 h-6 text-red-600 transition-all duration-300 ${isMobileMenuOpen ? 'rotate-90' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
